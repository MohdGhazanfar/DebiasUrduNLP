...
def dedup(seq): #remove duplicate from a list
    seen = set()
    return [x for x in seq if not (x in seen or seen.add(x))]

def safe_word(w):  #validity of word ,present
...
class WordEmbedding:
    def __init__(self, fname): # for loading the WE
        self.thresh = None
        self.max_words = None
        self.desc = fname
    ...
    def load_text_format(self, fname): # loads WE from a text file in the text format
        vecs = []
        words = []
    ...
    def reindex(self): # mapping between words and their indices in metrix
    ...
    def v(self, word):  # to retrieve vector representation of a word
        return self.vecs[self.index[word]]

    def diff(self, word1, word2): #diff bet 2 words
    ...
    def shrink(self, numwords): # shrink less freq WE
        self.desc += ", shrink " + str(numwords)
        self.filter_words(lambda w: self.index[w]<numwords)

    def filter_words(self, test): # handles the criterion
    ...
    def save(self, filename): #save word and v
    ...
    def remove_directions(self, directions, numwords=None): #remove vector of wrd v n wrd pair
    ...
    def compute_neighbors_if_necessary(self, thresh, max_words): # to create analogy pairs
        thresh = float(thresh)
    ...
    def neighbors(self, word, thresh=1): # returns a list of words similar to a given word based on  cosine similarity.
        if word not in self.index:
            return []
        dots = self.vecs.dot(self.v(word))
        return [self.words[i] for i, dot in enumerate(dots) if dot >= 1 - thresh / 2]

    def more_words_like_these(self, words, topn=50, max_freq=100000):  # find words that are similar to a given list of words.
...
def doPCA(pairs, embedding, num_components = 10): # PCA nalysis on metrix of wrd diff
    matrix = []
    for a, b in pairs:
